# Assignment Extract (provenance note)
- file: ASSIGNMENTS/MARKETING/project_1.md · sha256 `b0ccd92e1cf964fec17e74dcfaa03833433f45154d3054d0f2461919ae30b85c` · ingested 2026-09-16T02:31:49Z · run `carpathia-solver-v4-run-20260916T023149Z-480`
- document type: markdown brief (serves Assignment.pdf role under solver spec consumes contract)
- extraction: full text (322 lines); anchors = heading paths; no images in document
- immutable: not edited; extracted notes copied below
# --- full text copy ---
Assignment 1: Strategy Foundation

Build the strategic foundation for your semester-long digital brand project

Submission date: 18th September 2026
Format: Individual assignment
Submission: Presentation - PPT/PDF/Google slides/ Canva
No recommended length of slides, but an outline is given below. Feel free to increase/ decrease
the number of slides as per your content.

Important: The brand selected for this assignment will be used for subsequent assignments
throughout the semester.

# 1. Choosing Your Brand (most important)

You may choose either a fictional brand or a real small/emerging brand.

The brand should have:

●  A clearly defined product or service
●  A specific category
●  A realistic target consumer
●  A clear reason for existing / problem it solves
●  Enough scope for future digital marketing across content, social, search, paid media, CRM, etc.

## Good examples

●  Sustainable skincare brand
●  Premium coffee subscription
●  Athleisure label
●  Healthy snack brand
●  Travel planning app
●  Pet-care service
●  D2C home décor brand
●  Mental wellness app
●  Sneaker brand / marketplace
●  Affordable beauty brand

## Avoid

Ideas dependent on unrealistic technology

●  Extremely broad concepts such as “a fashion brand for everyone”
●  Brands with no clear product/service
●  Copying an existing major brand with only a new name
●  Categories where the you may have no clear understanding of the consumer
●  Creating multiple unrelated product categories under one brand
●  Not alcohol brands

## Brand Selection Test

Before finalising the brand, you should be able to answer:

  What do you sell?
  Who is it for?
  What problem does it solve?
  Why would someone choose you?

If these four answers aren't clear, your brand needs refinement.

—----- —--

# Suggested Slide Structure

## Slide 1 - Brand Introduction

Include:

●  Brand Name
●  Category
●  Product / Service
●  Geography / Market
●  Price Positioning
●  One-line description

(no detailed logo/ brand identity work is required at this stage)

## Slide 2 - Brand Overview
Explain briefly:

●  What does the brand offer?
●  What consumer problem does it solve?
●  What makes it different?
●  Who are the major competitors or alternatives?

(this will be more strategic than creative / design heavy)

## Slide 3 - Business Goal

Define one primary business goal.

Examples:

●  Generate ₹50 lakh in D2C revenue in Year 1.
●  Acquire 10,000 paying users within the first year.
●  Establish the brand in Mumbai and Bengaluru during the first 12 months.

(The business goal should describe what the business wants to achieve, not what marketing
will do)

## Slide 4 - Marketing Objective

Translate the business goal into 1 - 2 SMART marketing objectives.

Example:

Generate 5,000 qualified website visitors per month within six months and convert at least
4% into first-time customers.

You must explain briefly: How does this marketing objective support the business goal?

## Slide 5 - Target Audience

Define the primary target audience.

Include:

### Demographics

●  Age
●  Gender, if relevant
●  Location
●  Occupation / life stage

### Income / spending power

### Psychographics

●  Interests
●  Lifestyle
●  Values
●  Motivations
●  Attitudes

### Behaviour

●  Shopping habits
●  Digital behaviour
●  Existing alternatives used
●  Purchase frequency

Avoid descriptions like: “18 - 45, everyone interested in fitness.”

The audience should be specific enough to guide marketing decisions.

## Slide 6 - Buyer Persona

Create one detailed primary buyer persona. (You may add images/ creatives/ media to
emphasise the buyer persona)

### Include:

●  Name / profile
●  Age
●  Occupation
●  Location
●  Lifestyle
●  Interests
●  Goals
●  Pain points
●  Motivations
●  Favourite digital platforms
●  Online buying behaviour
●  One quote capturing their mindset

The persona should be based on the target audience - not random.

## Slide 7 - Customer Journey

Map the persona's journey from:

Need / Trigger → Discovery → Research → Consideration → Purchase → Experience →
Loyalty / Advocacy

For every stage, identify:

●  Consumer behaviour
●  Digital touchpoints
●  Questions / concerns
●  Potential friction

Example:

Instagram Reel → Google Search → Brand Website → Reviews → Purchase → Email →
Repeat Purchase

## Slide 8 - Digital Marketing Strategy

Write 1 - 2 concise paragraphs answering:

How will your brand use digital marketing to achieve its marketing objective?

The strategy should cover:

●  Who you are targeting
●  What you want them to think/do
●  Broad approach to reaching them
●  How digital channels will work together

It should not simply be a list of platforms.

For example:

❌ “We will use Instagram, Google and influencers.”

✅ “The brand will build awareness through creator-led educational content, capture high-intent
consumers through search, and use CRM to convert first-time buyers into repeat customers.”

## Slide 9 - PESO Media Mix

Map the brand across:

-  Paid

Examples: Meta Ads, Google Ads, paid creators.

-  Earned

Examples: PR, reviews, editorial coverage.

-  Shared

Examples: UGC, community posts, social shares.

-  Owned

Examples: Website, email database, social handles, app.

You do not need to add detailed campaigns yet.
The objective is to show that you understand how the four media types can work together.

## Slide 10 - Recommended Digital Channels

Choose approximately 4 - 6 priority digital channels.

For each, explain (for example) :

  Channel

Instagram

    Role

      Why This Channel?

Awareness

Target audience uses it heavily for discovery

Google Search

Intent

Captures consumers actively searching

Website

Email

Conversion

Retention

Primary purchase destination

Encourages repeat purchase

The important word here is recommended.

You should also be able to explain why certain platforms are not priorities.

## Slide 11 - Integrated Digital Journey
Bring everything together visually.

For example:

Instagram / Creator
 → Google Search
 → Website
 → Purchase
 → Email
 → Repeat Purchase
 → UGC / Referral

This slide should demonstrate that channels are connected, not isolated.

## Slide 12 - Key Strategic Decisions

End with 3 - 5 statements summarising the strategy.

For example:

●  Primary consumer = time-poor urban professionals.
●  Convenience is the key purchase driver.
●  Website UX must reduce purchase friction.
●  CRM will be critical for repeat purchases.

Instagram drives discovery; Search captures intent.

This will help you to articulate your actual strategic choices.

– —-- —--

Because this is only Assignment 1, do not include -

●  Detailed social media posts
●  Content calendars
●  Influencer lists
●  SEO keyword plans
●  Paid media budgets
●  Email journeys
●  Campaign creatives
●  Detailed analytics dashboards

Those will come up in later assignments, once the digital strategy for the brand is complete.

This assignment should establish who, why, what and where. Later assignments will build the
how.

Assignment 1 will primarily be marked on clarity of thinking, not creative / visual polish.


